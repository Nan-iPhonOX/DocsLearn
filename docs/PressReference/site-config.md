...
配置文件总是从 `<root>/.vitepress/config.[ext]` 解析，其中 `<root>` 是 VitePress [项目根目录](../PressGuide/routing#root-and-source-directory)，`[ext]` 是支持的文件扩展名之一。开箱即用地支持 TypeScript。支持的扩展名包括 `.js`、`.ts`、`.mjs` 和 `.mts`。
...
当设置为 `true` 时，VitePress 将从 URL 中删除 `.html` 后缀。另请参阅[生成简洁的 URL](../PressGuide/routing#generating-clean-url)。
...
自定义目录 &lt;-&gt; URL 映射。详细信息请参阅[路由：路由重写](../PressGuide/routing#route-rewrites)。
...
相对于项目根目录的 markdown 文件所在的文件夹。另请参阅[根目录和源目录](../PressGuide/routing#root-and-source-directory)。
...
项目的构建输出位置，相对于[项目根目录](../PressGuide/routing#root-and-source-directory)。
...
缓存文件的目录，相对于[项目根目录](../PressGuide/routing#root-and-source-directory)。另请参阅：[cacheDir](https://vitejs.dev/config/shared-options.html#cachedir)。
...
设置为 `true` 时，生产应用程序将在 [MPA 模式](../PressGuide/mpa-mode)下构建。MPA 模式默认提供 零 JavaScript 支持，代价是禁用客户端导航，并且需要明确选择加入才能进行交互。
