...
guide/index.md            -->../PressGuide/index.html (可以通../PressGuide/ 访问)
guide/getting-started.md  -->../PressGuide/getting-started.html
```
...
源目录是 Markdown 源文件所在的位置。默认情况下，它与项目根目录相同。但是，可以通过 [`srcDir`](../PressReference/site-config#srcdir) 配置选项对其进行配置。
...
[Getting Started](../PressGuide/getting-started)
...
如果可以使用此功能，还可以启用 VitePress 自己的 [`cleanUrls`](../PressReference/site-config#cleanurls) 配置选项，以便：
...
可以通过像这样配置 [`rewrites`](../PressReference/site-config#rewrites) 选项来实现此目的：
...
还可以通过 [`useData`](../PressReference/runtime-api#usedata) 运行时 API 访问当前页面的参数。这在 Markdown 文件和 Vue 组件中都可用：
