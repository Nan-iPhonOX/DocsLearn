...
还可以访问 VitePress 的运行时 API，例如 [`useData` 辅助函数](../PressReference/runtime-api#usedata)，它提供了当前页面的元数据：
...
Vitepress 目前只有使用 teleport 传送到 body 的 SSG 支持。对于其他地方，可以将它们包裹在内置的 `<ClientOnly>` 组件中，或者通过 [postRender 钩子](../PressReference/site-config#postrender)将 teleport 标签注入到最终页面 HTML 中的正确位置。
