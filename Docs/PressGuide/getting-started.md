...
:::tip
默认情况下，VitePress 将其开发服务器缓存存储在 `.vitepress/cache` 中，并将生产构建输出存储在 `.vitepress/dist` 中。如果使用 Git，应该将它们添加到 `.gitignore` 文件中。也可以手动[配置](../PressReference/site-config#outdir)这些位置。
:::
...
还可以通过 `themeConfig` 选项配置主题的行为。有关所有配置选项的完整详细信息，请参见[配置参考](../PressReference/site-config)。
...
更多的命令行用法请参见 [CLI 参考](../PressReference/cli)。
...
- 要探索默认文档主题提供的功能，请查看[默认主题配置参考](../PressReference/default-theme-config)。
